...
//    }
    ...
    // 지정된 게시물의 모든 댓글을 가져오는 메서드
    @GetMapping("/comment/{cno}")  // /comment/1   GET
    public ResponseEntity<CommentDto> read(@PathVariable("cno") int cno) {
        CommentDto dto = null;
        try {
            dto = service.read(cno);
            return new ResponseEntity<CommentDto>(dto, HttpStatus.OK);  // 200
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<CommentDto>(HttpStatus.BAD_REQUEST); // 400
        }
    }
    @GetMapping("/comments/test")
    public ResponseEntity<String> test() {
        return  new ResponseEntity<String>("{good:'verygood'}", HttpStatus.OK);  // 200
